...
class MultiscaleParallelCNF(nn.Module):
    ...
    def _logdensity(self, x, logpx=None, integration_times=None, update_t=False):
        ...
        for idx in range(len(self.transforms)):
            if not update_t:
                x, _logpx = self.transforms[idx].forward(x, _logpx, integration_times=integration_times,
                                                         update_t=update_t)
            else:
                x, _logpx, z_diff, logpz_diff = self.transforms[idx].forward(x, _logpx, integration_times=integration_times,
                                                         update_t=update_t)
#            x, _logpx = self.transforms[idx].forward(x, _logpx, integration_times=integration_times, update_t=update_t)
        if not update_t:
            return x if logpx is None else (x, _logpx)
        else:
            return (x, z_diff, logpz_diff) if logpx is None else (x, _logpx, z_diff, logpz_diff)
#        return x if logpx is None else (x, _logpx)
    ...
    def _generate(self, z, logpz=None, integration_times=None, update_t=False):
        ...
        for idx in reversed(range(len(self.transforms))):
            if not update_t:
                z, _logpz = self.transforms[idx](z, _logpz, reverse=True, integration_times=integration_times,
                                                 update_t=update_t)
            else:
                z, _logpz, z_diff, logpz_diff = self.transforms[idx](z, _logpz, reverse=True,
...
#                                             update_t=update_t)
        if not update_t:
            ...
            return (z, z_diff, logpz_diff) if logpz is None else (z, _logpz, z_diff, logpz_diff)
#        return z if logpz is None else (z, _logpz)

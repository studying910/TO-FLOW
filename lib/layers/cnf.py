...
class CNF(nn.Module):
    ...
    def forward(self, z, logpz=None, integration_times=None, reverse=False, update_t=False):
        ...
        if not update_t:
            ...
            z_diff, logpz_diff = self.get_z_logp_diff(integration_times[-1], z_t, logpz_t)
            if logpz is not None:
                return z_t, logpz_t, z_diff, logpz_diff
            else:
                return z_t, z_diff, logpz_diff

    def get_z_logp_diff(self, t1, z_t1, logpz_t1):
        # 公式中的\partial z_t1 / \partial t1   与 =Tr() ,    源码中取的z_t1而不是z_t0
        z_diff, logp_diff = self.odefunc(t1, (z_t1, logpz_t1))
        return z_diff, logp_diff
